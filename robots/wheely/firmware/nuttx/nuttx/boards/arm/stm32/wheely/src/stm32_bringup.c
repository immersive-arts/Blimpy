...
int stm32_bringup(void)
{
  ...
  if (ret < 0)
    {
      syslog(LOG_ERR, "ERROR: stm32_mpu6050() failed: %d\n", ret);
      return ret;
    }
  ...
  ret = board_qencoder_initialize(0, 2);
  if (ret != OK)
    {
      syslog(LOG_ERR,
  ...
  ret = board_qencoder_initialize(1, 3);
  if (ret != OK)
    {
      syslog(LOG_ERR,
             "ERROR: Failed to register the qencoder: %d\n",
  ...
  ret = board_qencoder_initialize(2, 4);
  if (ret != OK)
    {
      syslog(LOG_ERR,
             "ERROR: Failed to register the qencoder: %d\n",
  ...
  ret = board_qencoder_initialize(3, 5);
  if (ret != OK)
    {
      syslog(LOG_ERR,
             "ERROR: Failed to register the qencoder: %d\n",
	...
	stm32_timer_initialize ("/dev/timer0", 7);
#endif
